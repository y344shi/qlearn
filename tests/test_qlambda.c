/*
 * test_qlambda.c - validates Watkins's Q(lambda) (algo_qlambda.h) on the two
 * contest tasks (windy maze, phone DVFS). Copied from tests/test_reference.c;
 * the only change is the agent constructor (qlambda_agent_make).
 *
...
int main(void){
    ...
    rl_agent a1 = qlambda_agent_make(we.spec, we.n_features, we.n_actions, 123);
    ...
    rl_agent a2 = qlambda_agent_make(pe.spec, pe.n_features, pe.n_actions, 55);
