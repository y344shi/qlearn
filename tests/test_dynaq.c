/*
 * test_dynaq.c - validates the Dyna-Q agent (algo_dynaq.h) on the windy maze
 * and phone DVFS tasks. Follows the template in tests/test_reference.c.
 *
 *   gcc-12 -O2 -std=c99 -Wall -Wextra -Iinclude -o /tmp/test_dynaq tests/test_dynaq.c && /tmp/test_dynaq
 */
#include <stdio.h>
#include "rl_env.h"
...
int main(void){
    ...
    rl_agent a1 = dynaq_agent_make(we.spec, we.n_features, we.n_actions, 123);
    ...
    rl_agent a2 = dynaq_agent_make(pe.spec, pe.n_features, pe.n_actions, 55);
