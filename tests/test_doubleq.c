/*
 * test_doubleq.c - validates the Double Q-learning agent on the windy maze and
 * phone frequency-tuning tasks. Copied from tests/test_reference.c, swapping in
 * doubleq_agent_make. PASS criteria:
...
int main(void){
    ...
    rl_agent a1 = doubleq_agent_make(we.spec, we.n_features, we.n_actions, 123);
    ...
    rl_agent a2 = doubleq_agent_make(pe.spec, pe.n_features, pe.n_actions, 55);
