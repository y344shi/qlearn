/*
 * test_sarsalambda.c - validates the SARSA(lambda) agent (algo_sarsalambda.h)
 * on the two mandated contest tasks: windy maze (episodic) and phone DVFS
 * (continuing). Copied from tests/test_reference.c with sarsalambda_agent_make
 * swapped in for the reference Q-learning constructor. Pure integer; prints
 * PASS / exits 0 on success.
 *
 *   gcc-12 -O2 -std=c99 -Wall -Wextra -Iinclude -o /tmp/test_sarsalambda tests/test_sarsalambda.c && /tmp/test_sarsalambda
 */
#include <stdio.h>
#include "rl_env.h"
...
int main(void){
    ...
    rl_agent a1 = sarsalambda_agent_make(we.spec, we.n_features, we.n_actions, 123);
    ...
    rl_agent a2 = sarsalambda_agent_make(pe.spec, pe.n_features, pe.n_actions, 55);
