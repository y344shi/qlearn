/*
 * test_psweep.c - validates the Prioritized Sweeping agent on the windy maze and
 * phone frequency-tuning tasks. Copied from tests/test_reference.c, swapping in
 * psweep_agent_make. PASS criteria:
 *   - windy : reaches the goal in <= 2*opt + 5 steps
 *   - phone : jank <= oracle + 2%, energy < 95% of the performance governor
 *
 *   gcc-12 -O2 -std=c99 -Wall -Wextra -Iinclude -o /tmp/test_psweep \
 *       tests/test_psweep.c && /tmp/test_psweep
 */
#include <stdio.h>
#include "rl_env.h"
...
int main(void){
    ...
    rl_agent a1 = psweep_agent_make(we.spec, we.n_features, we.n_actions, 123);
    ...
    rl_agent a2 = psweep_agent_make(pe.spec, pe.n_features, pe.n_actions, 55);
