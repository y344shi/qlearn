/*
 * test_nstep.c - validates the n-step SARSA agent (algo_nstep.h) on the two
 * mandated contest tasks: windy maze (episodic) and phone DVFS (continuing).
 * Copied from tests/test_reference.c with nstep_agent_make swapped in for the
 * reference Q-learning constructor. Pure integer; prints PASS / exits 0.
 *
...
int main(void){
    ...
    rl_agent a1 = nstep_agent_make(we.spec, we.n_features, we.n_actions, 123);
    /* On-policy n-step SARSA explores via OPTIMISTIC INITIALISATION (see
       algo_nstep.h), so the behaviour policy here is greedy (eps 0 -> 0); the
       optimism alone drives the systematic exploration that finds the goal. */
    ...
    rl_agent a2 = nstep_agent_make(pe.spec, pe.n_features, pe.n_actions, 55);
