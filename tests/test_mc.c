/*
 * test_mc.c - validates the Monte-Carlo control agent (algo_mc.h) on the two
 * mandated contest tasks: windy maze (episodic) and phone DVFS (continuing).
 * Copied from tests/test_reference.c with mc_agent_make swapped in for the
 * reference Q-learning constructor. Pure integer; prints PASS / exits 0 on
 * success.
 *
 *   gcc-12 -O2 -std=c99 -Wall -Wextra -Iinclude -o /tmp/test_mc tests/test_mc.c && /tmp/test_mc
 */
#include <stdio.h>
#include "rl_env.h"
#include "algo_mc.h"
...
int main(void){
    ...
    rl_agent a1 = mc_agent_make(we.spec, we.n_features, we.n_actions, 123);
    /* Monte-Carlo needs many sampled episodes (no bootstrapping): train for the
       agent's full anneal horizon (see MC_ANNEAL_HI in algo_mc.h) so the value
       surface converges and freezes. */
    rl_train(&we, &a1, 8000000, 400, 10);
    ...
    rl_agent a2 = mc_agent_make(pe.spec, pe.n_features, pe.n_actions, 55);
