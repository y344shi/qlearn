/*
 * test_expsarsa.c - validates the Expected SARSA agent on the windy maze and
 * phone DVFS tasks. Mirrors tests/test_reference.c but uses
 * expsarsa_agent_make(). Pure integer; prints PASS and exits 0 on success.
 *
...
int main(void){
    ...
    rl_agent a1 = expsarsa_agent_make(we.spec, we.n_features, we.n_actions, 123);
    ...
    rl_agent a2 = expsarsa_agent_make(pe.spec, pe.n_features, pe.n_actions, 55);
